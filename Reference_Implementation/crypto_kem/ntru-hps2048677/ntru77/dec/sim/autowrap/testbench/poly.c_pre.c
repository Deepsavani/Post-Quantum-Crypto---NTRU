# 1 "C:/Users/deeps/OneDrive/Desktop/NTRU/NTRU/Reference_Implementation/crypto_kem/ntru-hps2048677/poly.c"
# 1 "C:/Users/deeps/OneDrive/Desktop/NTRU/NTRU/Reference_Implementation/crypto_kem/ntru-hps2048677/poly.c" 1
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 147 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "C:/Users/deeps/OneDrive/Desktop/NTRU/NTRU/Reference_Implementation/crypto_kem/ntru-hps2048677/poly.c" 2
...
# 2 "C:/Users/deeps/OneDrive/Desktop/NTRU/NTRU/Reference_Implementation/crypto_kem/ntru-hps2048677/poly.c" 2
...
# 3 "C:/Users/deeps/OneDrive/Desktop/NTRU/NTRU/Reference_Implementation/crypto_kem/ntru-hps2048677/poly.c" 2
# 1 "C:/Users/deeps/OneDrive/Desktop/NTRU/NTRU/Reference_Implementation/crypto_kem/ntru-hps2048677/verify.h" 1
...
# 5 "C:/Users/deeps/OneDrive/Desktop/NTRU/NTRU/Reference_Implementation/crypto_kem/ntru-hps2048677/verify.h" 2
...
# 4 "C:/Users/deeps/OneDrive/Desktop/NTRU/NTRU/Reference_Implementation/crypto_kem/ntru-hps2048677/poly.c" 2
...
void poly_Rq_mul(poly *r, const poly *a, const poly *b)
{
  ...
  for(k=0; k<677; k++)
  {
    r->coeffs[k] = 0;
    for(i=1; i<677 -k; i++)
      r->coeffs[k] += a->coeffs[k+i] * b->coeffs[677 -i];
    for(i=0; i<k+1; i++)
...
void poly_Sq_mul(poly *r, const poly *a, const poly *b)
{
  ...
  poly_Rq_mul(r, a, b);
  for(i=0; i<677; i++)
    r->coeffs[i] = ((r->coeffs[i] - r->coeffs[677 -1]) & ((1 << 11)-1));
}
...
void poly_S3_mul(poly *r, const poly *a, const poly *b)
{
  ...
  for(k=0; k<677; k++)
    r->coeffs[k] = mod3(r->coeffs[k] + 2*r->coeffs[677 -1]);
}

void poly_Rq_mul_x_minus_1(poly *r, const poly *a)
{
  int i;
  uint16_t last_coeff = a->coeffs[677 -1];
...
# 150 "C:/Users/deeps/OneDrive/Desktop/NTRU/NTRU/Reference_Implementation/crypto_kem/ntru-hps2048677/poly.c"
void poly_Rq_to_S3(poly *r, const poly *a)
{
  ...
  r->coeffs[677 -1] = mod3(r->coeffs[677 -1]);
  for(i=0; i<677; i++)
    r->coeffs[i] = mod3(r->coeffs[i] + 2*r->coeffs[677 -1]);
}
# 176 "C:/Users/deeps/OneDrive/Desktop/NTRU/NTRU/Reference_Implementation/crypto_kem/ntru-hps2048677/poly.c"
...
static inline void poly_divx(poly *a, int s)
{
  ...
  a->coeffs[677 -1] = (!s * a->coeffs[677 -1]);
}
...
static inline void poly_mulx(poly *a, int s)
{
  ...
  for(i=1; i<677; i++)
    a->coeffs[677 -i] = (s * a->coeffs[677 -i-1]) | (!s * a->coeffs[677 -i]);
...
static void poly_R2_inv(poly *r, const poly *a)
{
  ...
  for(j=0;j<2*(677 -1)-1;j++)
  {
    ...
    for(i=0; i<677; i++) { f.coeffs[i] ^= g.coeffs[i] * sign*(!done); };
    for(i=0; i<677; i++) { b.coeffs[i] ^= (*c).coeffs[i] * sign*(!done); };
  ...
  for (i = 0; i < 10; i++) {
    for (j = 0; j < 677; j++) {
      temp_r->coeffs[j] = r->coeffs[(j + (1 << i)) % 677];
    }
    cmov((unsigned char *)&(r->coeffs),
         (unsigned char *)&(temp_r->coeffs), sizeof(uint16_t) * 677, k & 1);
    k >>= 1;
  }
}
...
void poly_S3_inv(poly *r, const poly *a)
{
  ...
  for(j=0; j<2*(677 -1)-1; j++)
  {
    ...
    for(i=0; i<677; i++) { f.coeffs[i] = mod3(f.coeffs[i] + sign*(!done) * g.coeffs[i]); };
    for(i=0; i<677; i++) { b.coeffs[i] = mod3(b.coeffs[i] + sign*(!done) * c.coeffs[i]); };
